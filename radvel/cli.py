...
def main():
    ...
    # Fitting
    psr_fit2 = subpsr.add_parser(
        'fit2', parents=[psr_parent],
        description="Perform Powell fitting"
    )
    psr_fit2.set_defaults(func=radvel.driver.fit2)
    ...
    psr_mcmc.add_argument(
    '--act', dest='autocorrrelthreshmax', action='store', default=0.01, type=float,
    help="Maximum tolerance allowed on change in autocorrelation scale between convergence checks to consider the MCMC converged [0.01]"
    )
    psr_mcmc.add_argument(
    '--ac', dest='autocorrmin', action='store', default=100, type=int,
    help="Minimum autocorrelation scale required to consider the MCMC converged [100]"
    )
    psr_mcmc.add_argument(
    '--minsteps', dest='minsteps', action='store', default=100, type=int,
    help="Minimum number of steps per walker run at a time [100]"
    )
    psr_mcmc.add_argument(
    ...
    psr_mcmc.add_argument(
    '--nburn', dest='nburn', action='store', default=0, type=int,
    help="Number of samples to burn in computation [default=0, save all samples]"
    )
    psr_mcmc.add_argument(
    '--checkinterval', dest='checkinterval', action='store', default=200, type=int,
    help="Minimum number of steps per walker before convergence tests are performed [100]"
    )
